...
# 주의할점 : 그룹 기준 외 다른 속성들의 대해 어떻게 보여줄지 정의 안됨. ( 조회 결과는 무조건 표/테이블 )
...
# [2] 기본 집계 함수 
select sum( bamount ) from buy; -- buy 테이블내 bamount 속성값 들의 전체 합계
select avg( bamount ) from buy; -- buy 테이블내 bamount 속성값 들의 전체 평균
select min( bamount ) from buy; -- buy 테이블내 bamount 속성값 들의 전체 중 최솟값 
select max( bamount ) from buy; -- buy 테이블내 bamount 속성값 들의 전체 중 최댓값
select count( bamount ) from buy; -- buy 테이블내 bamount 속성값 들의 전체 레코드 수 ( null 제외 )
select count( * ) from buy;	-- buy 테이블내 bamount 속성값 들의 전체 레코드 수 ( null 포함 )
