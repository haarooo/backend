...
/*[문제 1] 정수 5개(10, 20, 30, 40, 50)를 저장할 수 있는
int 타입의 배열 numbers1를 선언과 동시에 초기화하세요.
...
선언 코드: int[] scores2 = {77, 82, 100, 54, 96};*/
        ...
        for(int score : scores2){
            if(score ==100){
            System.out.println("만점자가 나타났다");break;
            }
        }
...
int[] stock = {10, 5, 20};*/
        ...
        System.out.print("수량 : "); int count1 = scan.nextInt();
        int check = 0;
        for( int index = 0  ; index <= products.length-1 ; index++){
            if(products[index].equals(name)){
                if(stock[index] >= count1){
                    stock[index] -= count1;
                    check =1 ; break;
                }
                else{
...
931나8234: 420분 주차, 최종 요금: 20000원*/
        ...
        for(int index = 0 ; index <= carNumbers.length-1 ; index++){
            ...
            int userTime = usageMinutes[index];
            if(userTime <= 10){money = 1000;
            }else{
                money = (userTime - 10)/10;
                money += 500;
                money += 1000;
            }
            money = money > 20000 ? 20000 : money;
            System.out.println(userTime + "분 주차 , 요금 :" + money+"원");
